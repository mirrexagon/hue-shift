...
class Block
    new: (game, color, x, y) =>
        ...
        -- This is modified in draw IF beat_fraction is supplied.
        -- Otherwise, it is free to be modified externally.
        @alpha = 0
    ...
    draw_at: (x, y, beat_fraction) =>
        -- Only update block alpha with beat if beat_fraction is supplied.
        if beat_fraction
            @alpha = @compute_beat_alpha beat_fraction
